...
import app.dataRecovery.MainDataRecovery.logger
...
object TwitterServiceOperations extends Logging with TwitterClientTrait with ValidationsUtilTrait with DatesUtilTrait {
  ...
  def getTweetText(tweets: java.util.ArrayList[String], textColumn: Int): mutable.Buffer[String] = {
    tweets.map({ tweet =>
      try {
        tweet.split("\t")(textColumn).replace("\"", "")
      }
      catch {
        case exception: Exception => {
          logger.debug("Tweet could not be parsed: " + tweet)
          tweet
        }
      }
    })
